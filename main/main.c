...
// MQTT
#define EXAMPLE_ESP_MQTT_URL CONFIG_MQTT_BROKER_URL
#define EXAMPLE_ESP_MQTT_TOKEN CONFIG_MQTT_TOKEN
...
static esp_err_t mqtt_event_handler_cb(esp_mqtt_event_handle_t event)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        ESP_LOGI(TAG, "send attributes to thingsboard server");
        
        // test send attributes and telemetry
        msg_id = esp_mqtt_client_publish(client, PUBLISH_ATTIBUTES, "{\"ip\":\"10.10.10.10\"}", 0, 1, 0);
        msg_id = esp_mqtt_client_publish(client, PUBLISH_TELEMETRY, "{\"temperature\":42}", 0, 1, 0);
        
        // subscribe to the RPC chanel
        ESP_LOGI(TAG, "Subscribe to RPC");
        msg_id = esp_mqtt_client_subscribe(client, SERVER_RPC, 1);
    ...
    case MQTT_EVENT_DATA:
        // new incomming data here
        // TODO: handle PRC command
        ...
        // just test
        msg_id = esp_mqtt_client_publish(client, topic_response, "{\"ack\":\"lala\"}", 0, 1, 0);
...
static void mqtt_app_start(void)
{
    esp_mqtt_client_config_t mqtt_cfg = {
        .host = EXAMPLE_ESP_MQTT_URL,
        .username = EXAMPLE_ESP_MQTT_TOKEN,
        .port = 1883,
    };
...
static esp_err_t get_esp32_mac(uint8_t *mac)
{
...
void app_main(void)
{
    ...
    get_esp32_mac(baseMac);
    ESP_LOGI(TAG, "ID: %02X%02X%02X%02X%02X%02X", baseMac[0], baseMac[1], baseMac[2], baseMac[3], baseMac[4], baseMac[5]);
    test_json();
